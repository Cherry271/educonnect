...
import { Mail, Lock, Eye, EyeOff, GraduationCap, User, Shield, Check, Globe, MessageSquare } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const registerData = {
        ...
        password: password,
        role: role,
        department: role === 'student' ? 'Computer Science' : 'Science Department'
      };
      ...
      console.log('Backend register failed, trying offline mock registration...', err);
      ...
      setTimeout(() => {
        ...
        const newMockUser = {
          ...
          email: email,
          role: role,
          department: role === 'student' ? 'Bio-Engineering' : 'Quantum Computing',
          faculty: 'Science & Engineering',
          bio: `Hello! I am ${firstName}, newly registered as a ${role} on EduConnect.`,
          profile_picture: '',
          cover_photo: '',
          skills: ['Learning', 'Collaboration'],
          interests: ['Research'],
          followers_count: 0,
          following_count: 0,
          posts_count: 0,
          resources_count: 0,
          achievements: [{ title: role === 'student' ? 'Fresh Member' : 'Educator' }],
          created_at: new Date().toISOString()
        };

        setTokens('mock-register-access-token', 'mock-register-refresh-token');
        setUser(newMockUser);
        toast.success(`Registered successfully as ${firstName} (Offline Mode)`);
        setIsLoading(false);
      }, 1000);
      return;
    }
    ...
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2 bg-[#121413] select-none text-gray-100 font-sans">
      ...
      <div className="hidden lg:flex flex-col justify-between p-12 bg-[#3ddc97] text-dark-bg relative overflow-hidden select-none">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-xl tracking-wide text-dark-bg">EduConnect</span>
        </div>
        ...
        <div className="max-w-md my-auto space-y-6">
          ...
          <p className="text-dark-bg/80 text-base font-semibold leading-relaxed">
            The global hub for students and educators to collaborate, share, and grow together.
          </p>
        </div>
      ...
      <div className="flex flex-col justify-center p-6 sm:p-12 overflow-y-auto">
        <div className="w-full max-w-[440px] mx-auto space-y-6">
          ...
          {/* Header */}
          <div>
            <h1 className="text-2xl font-black text-white tracking-wide">Create Account</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  <User size={14} className={role === 'student' ? 'text-[#3ddc97]' : 'text-gray-500'} />
                  Student
                </button>
                <button
                  type="button"
                ...
                >
                  <Shield size={14} className={role === 'teacher' ? 'text-[#3ddc97]' : 'text-gray-500'} />
                  Teacher
                </button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-600 text-xs font-bold font-mono">@</span>
                <input
                  type="email"
            ...
            <div className="flex items-start gap-3 pt-2">
              <button
                ...
                className={`w-5.5 h-5.5 rounded-full border flex items-center justify-center shrink-0 transition-all cursor-pointer ${
                  agreeTOS ? 'bg-[#3ddc97] border-[#3ddc97] text-dark-bg' : 'border-gray-800 hover:border-gray-600'
                }`}
              >
                {agreeTOS && <Check size={12} className="stroke-[3]" />}
              </button>
              <div className="text-[11px] leading-relaxed text-gray-400">
                <span className="text-gray-200 font-semibold">I agree to the Terms of Service</span>
                <p className="text-[10px] text-gray-500">Read our privacy policy regarding data usage</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              <button
                ...
                className={`w-5.5 h-5.5 rounded-full border flex items-center justify-center shrink-0 transition-all cursor-pointer ${
                  subscribeNews ? 'bg-[#3ddc97] border-[#3ddc97] text-dark-bg' : 'border-gray-800 hover:border-gray-600'
                }`}
              >
                {subscribeNews && <Check size={12} className="stroke-[3]" />}
              </button>
              <div className="text-[11px] leading-relaxed text-gray-400">
                <span className="text-gray-200 font-semibold">Subscribe to newsletter</span>
                <p className="text-[10px] text-gray-500">Occasional updates on new features and resources</p>
              </div>
            </div>
          ...
          <div className="flex justify-center items-center gap-2 text-[10px] text-gray-500 pt-2 font-medium">
            <span className="w-4 h-4 rounded-full bg-emerald-950/40 border border-emerald-900/40 flex items-center justify-center text-primary-400 text-[8px] font-black shrink-0">✓</span>
